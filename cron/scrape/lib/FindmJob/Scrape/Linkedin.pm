...
sub run {
    ...
    foreach my $r (@{$data->{jobs}->{values}}) {
        ...
        unless (@tags) {
            my @bad_industries = ('Financial Services', 'Transportation/Trucking/Railroad', 'Hospital & Health Care', 'Pharmaceuticals', 'Biotechnology', 'Electrical/Electronic Manufacturing', 'Airlines/Aviation', 'Machinery', 'Chemicals', 'Industrial Automation', 'Food Production', 'Accounting', 'Real Estate', 'Wine and Spirits', 'Renewables & Environment', 'Health, Wellness and Fitness', 'Consumer Goods', 'Apparel & Fashion');
